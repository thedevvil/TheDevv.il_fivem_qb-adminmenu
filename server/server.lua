...
QBCore.Functions.CreateCallback('noclip:server:check', function(source, cb)
    local src = source
...
RegisterNetEvent('qb-admin:server:kick', function(player, reason)
    ...
    if QBCore.Functions.HasPermission(src, permissions['kick']) or IsPlayerAceAllowed(src, 'command')  then
        TriggerEvent('qb-log:server:CreateLog', 'bans', 'Oyuncu', 'red', string.format('%s tarafından tekmelendiği için Tekmelendi %s', GetPlayerName(player.id), GetPlayerName(src), reason), true)
...
RegisterNetEvent('qb-admin:server:ban', function(player, time, reason)
    ...
    if QBCore.Functions.HasPermission(src, permissions['ban']) or IsPlayerAceAllowed(src, 'command') then
      ...
      --  })
        TriggerEvent('qb-log:server:CreateLog', 'bans', 'Oyuncu Yasaklandı', 'red', string.format('%s %s tarafından yasaklandı %s', GetPlayerName(player.id), GetPlayerName(src), reason), true)
        if banTime >= 2147483647 then
...
RegisterNetEvent('qb-admin:server:discordal', function(id)
    for k,v in pairs(GetPlayerIdentifiers(id))do
     if string.sub(v, 1, string.len("discord:")) == "discord:" then
          discord = v
        end
    end
    TriggerClientEvent("qb-admin:client:discordal", source, discord)
end)
...
RegisterNetEvent('qb-admin:server:SaveCar', function(mods, vehicle, hash, plate)
    ...
    if result[1] == nil then
        ...
        TriggerClientEvent('QBCore:Notify', src, 'Artık aracın sahibisin', 'success', 5000)
    else
        TriggerClientEvent('QBCore:Notify', src, 'Zaten bu araç senin..', 'error', 3000)
    end
end)
...
QBCore.Commands.Add('duyuru', 'Duyuru mesajı', {}, false, function(source, args)
    local argString = table.concat(args, " ")
	TriggerClientEvent('rh:duyurualert', -1, {text = argString})
end, 'god')
...
QBCore.Commands.Add('reportr', 'Reply To A Report (Admin Only)', {{name='id', help='Player'}, {name = 'message', help = 'Message to respond with'}}, false, function(source, args, rawCommand)
    ...
    TriggerEvent('qb-log:server:CreateLog', 'report', 'Yanıtı Bildir', 'red', '**'..GetPlayerName(src)..'** cevapladı: **'..OtherPlayer.PlayerData.name.. ' **(ID: '..OtherPlayer.PlayerData.source..') **Message:** ' ..msg, false)
end, 'admin')
...
QBCore.Commands.Add('bring', 'Kişiyi yanına çek', { { name = 'id', help = 'ID' }, }, false, function(source, args)
    if args[1] then
        local src = source
        local admin = GetPlayerPed(src)
        local coords = GetEntityCoords(admin)
