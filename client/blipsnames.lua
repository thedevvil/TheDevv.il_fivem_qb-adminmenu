...
RegisterNetEvent('qb-admin:client:toggleBlips', function()
    if not ShowBlips then
        ShowBlips = true
        NetCheck1 = true
        QBCore.Functions.Notify("Blipler Aktif!", "success")
    else
        ShowBlips = false
        QBCore.Functions.Notify("Blipler Aktif Değil!", "error")
    end
end)
...
RegisterNetEvent('qb-admin:client:toggleNames', function()
    if not ShowNames then
        ShowNames = true
        NetCheck2 = true
        QBCore.Functions.Notify("İsimler Açık!", "success")
    else
        ShowNames = false
        QBCore.Functions.Notify("İsimler Kapalı!", "error")
    end
end)
